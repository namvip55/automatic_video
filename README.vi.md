...
### Biến bài báo công nghệ tiếng Việt thành video dọc 9:16 chỉ trong 60 giây
...
- 🎬 Render video hoàn chỉnh → tự động
...
Cả 2 cách: sau ~3–5 phút bạn sẽ có `output/<slug>/video.mp4` — file 1080×1920 sẵn sàng cho Shorts / Reels / nền tảng video dọc.
...
<td width="33%" align="center">
<h3>📱 9:16 Native</h3>
<sub>1080×1920 @ 30fps, sẵn cho video dọc / Shorts / Reels</sub>
</td>
<td width="33%" align="center">
...
<td width="33%" align="center">
<h3>🎬 Output gọn</h3>
<sub>Bàn giao duy nhất <code>video.mp4</code>; các file audio/subtitle chỉ là artifact nội bộ</sub>
</td>
</tr>
</table>
...
flowchart LR
    ...
    F -->|audio nội bộ<br/>+ SFX mix<br/>+ subtitle burn-in| G[HyperFrames]
...
| **Visual blocks** | HyperFrames registry: `grain-overlay`, `shimmer-sweep` |
...
Nếu set, pipeline sinh thumbnail 9:16 cho mỗi video và embed làm cover image của MP4 — Windows Explorer / Finder / video dọc / YouTube uploader hiển thị ảnh đó trước khi play frame đầu. Không có key, step bị skip im lặng (video vẫn render bình thường).
...
├── voice/                     # TTS/SRT từng scene, artifact nội bộ
├── voice-raw.mp3              # Audio concat, artifact nội bộ
├── voice.mp3                  # Audio final dùng để render video, artifact nội bộ
├── subtitles.srt              # Subtitle merged trước khi burn-in
...
└── video.mp4                  # 🎉 Output cuối — 1080×1920 @ 30fps + phụ đề burn-in
```
...
Mỗi video gồm **persistent shell** xuyên suốt (header brand icon + tên channel + tag, footer handle video dọc, grain texture, gradient background) cộng với 4–18 scene Claude tự pick theo nội dung. Base palette luôn là **cream editorial (light)** để giữ brand identity nhất quán; field `theme` trên `script.metadata` đổi accent color:
...
HyperFrames được purpose-built cho short-form video — 9:16 native, 50+ blocks social media (video dọc cards, kinetic typography, data viz), AI-agent friendly (Claude có thể tự sinh HTML composition mà không cần React boilerplate).
...
- [ ] Auto-upload video dọc / YouTube Shorts / Reels qua API
